...
## [v2.0.1] - 2026-06-23
### Fixed
- Stored `company_id` as an integer in `/fp/auto_install` sessions, matching `/fp/install` and avoiding mixed string/number values in extension storage.

## [v2.0.0] - 2026-04-14
### Added
- Signature verification for extension proxy requests using `fp-signature` library.
- SDK version header for signature.
- Transformation support for proxy request payloads.
- Error log messages for signature verification failures.

### Changed
- Removed query dependency from signature handling.
