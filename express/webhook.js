...
class WebhookRegistry {
  ...
  async initialize(config, fdkConfig) {
    ...
    if (config.marketplace === true && config.subscribed_saleschannel !== 'specific') {
      throw new FdkInvalidWebhookConfig(
        'marketplace is only allowed when subscribed_saleschannel is specific',
      );
    }
    ...
    for (const [eventName, eventData] of Object.entries(this._config.event_map)) {
      // Validate Webhook Event Map
      if (eventName.split('/').length !== 3) {
        throw new FdkInvalidWebhookConfig(
          `Invalid webhook event map key. Invalid key: ${eventName}`,
        );
      }
      ...
      if (!allowedProviders.includes(eventData.provider)) {
        throw new FdkInvalidWebhookConfig(
          `Invalid provider value in webhook event ${eventName}, allowed values are ${allowedProviders.toString()}`,
        );
      }
      if (eventData.provider === 'rest' && !eventData.hasOwnProperty('handler')) {
      ...
      } else if ((eventData.provider === 'kafka' || eventData.provider === 'pub_sub')
        && !eventData.hasOwnProperty('topic')) {
        throw new FdkInvalidWebhookConfig(
          `Missing topic in webhook event ${eventName}`,
        );
      } else if ((eventData.provider === 'temporal' || eventData.provider === 'sqs')
        && !eventData.hasOwnProperty('queue')) {
        throw new FdkInvalidWebhookConfig(
          `Missing queue in webhook event ${eventName}`,
        );
        ...
        throw new FdkInvalidWebhookConfig(
          `Missing event_bridge_name in webhook event ${eventName}`,
        );
      }

      if ('filters' in eventData && typeof eventData.filters !== 'object') {
        throw new FdkInvalidWebhookConfig(
          `filters should be an object in webhook event ${eventName}`,
        );
      }

      if ('reducer' in eventData && typeof eventData.reducer !== 'object') {
        throw new FdkInvalidWebhookConfig(
          `reducer should be an object in webhook event ${eventName}`,
        );
      }
    ...
    if (Object.keys(eventConfig.eventsNotFound).length) {
      ...
      throw new FdkInvalidWebhookConfig(
        `Webhooks events ${errors.join(' and ')} not found`,
      );
    }
  ...
  _getEventIdMap(events) {
    return events.reduce((eventMap, event) => {
      eventMap[`${event.event_category}/${event.event_name}/${event.event_type}/v${event.version}`] = event.id;
      return eventMap;
    }, {});
  }
  ...
  _isConfigUpdated(subscriberConfig) {
    ...
    if (configCriteria !== subscriberConfig.association.criteria) {
      ...
      logger.debug(
        `Webhook association criteria updated from ${subscriberConfig.association.criteria} to ${configCriteria}`,
      );
    ...
    if (this._config.notification_email !== subscriberConfig.email_id) {
      logger.debug(
        `Webhook notification email updated from ${subscriberConfig.email_id} to ${this._config.notification_email}`,
      );
    ...
    } else if (subscriberConfig.type === 'marketplace') {
      logger.debug(`Type updated from ${subscriberConfig.type} to null`);
      subscriberConfig.type = null;
      updated = true;
    }
  ...
  async syncEvents(platformClient, config = null, enableWebhooks) {
    ...
    const subscriberConfigList = await this.getSubscriberConfig(platformClient);

    const subscriberSyncedForAllProvider = await this.syncSubscriberConfigForAllProviders(
      platformClient,
    ...
    if (!subscriberSyncedForAllProvider) {
      const subscriberConfigListV2 = await this.getSubscriberConfig(platformClient);
      await this.syncSubscriberConfig(
        subscriberConfigListV2.rest,
        'rest',
        this._eventMap.rest,
        platformClient,
        enableWebhooks,
      ...
      await this.syncSubscriberConfig(
        subscriberConfigListV2.pub_sub,
        'pub_sub',
        this._eventMap.pub_sub,
        platformClient,
        enableWebhooks,
      ...
      await this.syncSubscriberConfig(
        subscriberConfigListV2.event_bridge,
        'event_bridge',
        this._eventMap.event_bridge,
        platformClient,
        enableWebhooks,
      ...
      await this.syncSubscriberConfig(
        subscriberConfigListV2.temporal,
        'temporal',
        this._eventMap.temporal,
        platformClient,
        enableWebhooks,
  ...
  async syncSubscriberConfigForAllProviders(platformClient, subscriberConfigList) {
    const payload = this.createRegisterPayloadData(subscriberConfigList);
    const uniqueKey = `registerSubscriberToEventForAllProvider_${platformClient.config.companyId}`
      + `_${this._fdkConfig.api_key}`;
    ...
    try {
      try {
        const rawRequest = {
          method: 'put',
          url:
            `${this._fdkConfig.cluster}/service/platform/webhook/v3.0/company/`
            + `${platformClient.config.companyId}/subscriber`,
          data: payload,
          headers: {
        ...
        // Check if it's a 404 error - could be in err.code or err.response.status
        if (err.code !== '404' && err.response?.status !== 404 && err.message?.includes('404') === false) {
          throw err;
        }
        return false;
      }
      ...
      if (
        ...
        return this._retryManager.retry(
          uniqueKey,
          this.syncSubscriberConfigForAllProviders.bind(this),
          platformClient,
        );
      }
      throw new FdkWebhookRegistrationError(
        `Error while updating webhook subscriber configuration for all providers. Reason: ${err.message}.`
        + ` Errors: ${JSON.stringify(err?.details)}`,
      );
    }
  }
  ...
  createRegisterPayloadData(subscriberConfigList) {
    ...
    const payloadEventMap = payload.webhook_config.event_map;
    for (const [key, event] of Object.entries(this._config.event_map)) {
      ...
      const [event_category, event_name, event_type] = key.split('/');
      const eventData = {
        event_category,
        event_name,
        event_type,
  ...
  async syncSubscriberConfig(subscriberConfig, configType, currentEventMapConfig, platformClient, enableWebhooks) {
    ...
    if (!subscriberConfig) {
      ...
      logger.debug(
        `Webhook ${configType} config on platform side for company id `
        + `${platformClient.config.companyId}: ${JSON.stringify(subscriberConfig)}`,
      );
    ...
    for (const eventName of Object.keys(currentEventMapConfig)) {
      const event_id = eventConfig.eventsMap[eventName];
      if (event_id) {
        const event = {
          slug: eventName,
    ...
    try {
      if (registerNew) {
        if (subscriberConfig.events.length === 0) {
          logger.debug(
            `Skipped registerSubscriber API call as no ${configType} based events found`,
          );
          return;
        }
        ...
        if (this._fdkConfig.debug) {
          ...
          logger.debug(
            `Webhook ${configType} config registered for company: ${platformClient.config.companyId}, `
            + `config: ${JSON.stringify(subscriberConfig)}`,
          );
        }
      } else {
        const eventDiff = [
        ...
        const commonKeys = ['filters', 'reducer'];
        ...
        if (!configUpdated) {
          for (const event of subscriberConfig.events) {
            ...
            if (existingEvent) {
              ...
              for (const key of configTypeKeysToCheck[configType]) {
        ...
        if (eventDiff.length || configUpdated) {
          ...
          if (this._fdkConfig.debug) {
            subscriberConfig.events = subscriberConfig.events?.map((event) => event.slug);
            logger.debug(
              `Webhook ${configType} config updated for company: ${platformClient.config.companyId}, `
              + `config: ${JSON.stringify(subscriberConfig)}`,
            );
          }
        }
  ...
  async enableSalesChannelWebhook(platformClient, applicationId) {
    ...
    try {
      ...
      for (const subscriberConfigType in subscriberConfigList) {
        ...
        const {
          id, name, webhook_url, provider = 'rest', association, status, auth_meta, event_configs, email_id,
        } = subscriberConfig;
        subscriberConfig = {
          id, name, webhook_url, provider, association, status, auth_meta, email_id,
        };
  ...
  async processWebhook(req) {
    ...
    try {
      ...
      if (typeof extHandler === 'function') {
        logger.debug(
          `Webhook event received for company: ${req.body.company_id}, `
          + `application: ${req.body.application_id || ''}, event name: ${eventName}`,
        );
    ...
    try {
      try {
        const rawRequest = {
          method: 'post',
          url:
            `${this._fdkConfig.cluster}/service/platform/webhook/v2.0/company/`
            + `${platformClient.config.companyId}/subscriber`,
        ...
        if (subscriberConfig.provider !== 'rest') {
          logger.debug(
            `Webhook Subscriber Config type ${subscriberConfig.provider} is not supported with current fp version`,
          );
          return;
        }
        if (err.code !== '404') {
        ...
        const { provider } = subscriberConfig;
        ...
        const rawRequest = {
          method: 'post',
          url:
            `${this._fdkConfig.cluster}/service/platform/webhook/v1.0/company/`
            + `${platformClient.config.companyId}/subscriber`,
          data: subscriberConfig,
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      throw new FdkWebhookRegistrationError(
        `Error while registering webhook subscriber configuration, Reason: ${err.message}`,
      );
    }
  }
  ...
  async updateSubscriberConfig(platformClient, subscriberConfig) {
    ...
    try {
      ...
      try {
        ...
        // Handle 404 errors gracefully (v2.0 API not found)
        if (err.code !== '404' && err.response?.status !== 404 && err.message?.includes('404') === false) {
          throw err;
        }
        const eventsList = subscriberConfig.events;
      ...
      throw new FdkWebhookRegistrationError(
        `Error while updating webhook subscriber configuration. Reason: ${err.message}`,
      );
    }
  }
  ...
  async getSubscriberConfig(platformClient) {
    ...
    try {
      const rawRequest = {
        ...
        url:
          `${this._fdkConfig.cluster}/service/platform/webhook/v1.0/company/`
          + `${platformClient.config.companyId}/extension/${this._fdkConfig.api_key}/subscriber`,
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
          'x-ext-lib-version': `js/${version}`,
      ...
      throw new FdkInvalidWebhookConfig(
        `Error while fetching webhook subscriber configuration, Reason: ${err.message}`,
      );
    }
  }

  async getEventConfig(handlerConfig) {
    const url = `${this._fdkConfig.cluster}/service/common/webhook/v1.0/events/query-event-details`;
    ...
    try {
      ...
      Object.keys(handlerConfig).forEach((key) => {
        const eventObj = {};
        const [eventCategory, eventName, eventType] = key.split('/');
        eventObj.event_category = eventCategory;
      ...
      const responseData = await fdkAxios.request(rawRequest);
      ...
      if (
        ...
        return this._retryManager.retry(uniqueKey, this.getEventConfig.bind(this), handlerConfig);
      }
