...
const {
  FdkWebhookProcessError, FdkWebhookHandlerNotFound, FdkWebhookRegistrationError, FdkInvalidHMacError, FdkInvalidWebhookConfig,
} = require('./error_code');
...
class WebhookRegistry {
  ...
  async syncEvents(platformClient, config = null, enableWebhooks) {
    ...
    const subscriberSyncedForAllProvider = await this.syncSubscriberConfigForAllProviders(platformClient, subscriberConfigList);
  ...
  async syncSubscriberConfig(subscriberConfig, configType, currentEventMapConfig, platformClient, enableWebhooks) {
    ...
    if (!subscriberConfig) {
      ...
      const {
        id, name, webhook_url, provider = 'rest', association, status, auth_meta, event_configs, email_id, type,
      } = subscriberConfig;
      subscriberConfig = {
        id, name, webhook_url, provider, association, status, auth_meta, email_id, type,
      };
