...
function setupRoutes(ext) {
  ...
  FdkRoutes.get('/fp/install', async (req, res, next) => {
    ...
    try {
      ...
      const platformConfig = await ext.getPlatformConfig(companyId);
      const redirectPath = req.query.redirect_path;

      const session = new Session(Session.generateSessionId(true));

      const sessionExpires = new Date(Date.now() + 900000); // 15 min
      ...
      const redirectUrl = platformConfig.oauthClient.startAuthorization({
        scope: session.scope,
  ...
  FdkRoutes.get('/fp/auth', sessionMiddleware(false), async (req, res, next) => {
    ...
    try {
      ...
      if (req.fdkSession.state !== req.query.state) {
        ...
        const compCookieName = `${SESSION_COOKIE_NAME}_${companyId}`;
        res.clearCookie(compCookieName, {
      ...
      const token = platformConfig.oauthClient.raw_token;
      const sessionExpires = new Date(Date.now() + token.expires_in * 1000);
      ...
      if (!ext.isOnlineAccessMode()) {
        ...
        const offlineTokenRes = await platformConfig.oauthClient.getOfflineAccessToken(ext.scopes, req.query.code);
  ...
  FdkRoutes.post('/fp/auto_install', sessionMiddleware(false), async (req, res, next) => {
    try {
      ...
      const sid = Session.generateSessionId(false, {
        cluster: ext.cluster,
        id: company_id, // eslint-disable-line camelcase
      });
      ...
      const offlineTokenRes = await platformConfig.oauthClient.getOfflineAccessToken(ext.scopes, code);

      session.company_id = company_id; // eslint-disable-line camelcase
      session.scope = ext.scopes;
      session.state = uuidv4();
      session.extension_id = ext.api_key;
      offlineTokenRes.access_token_validity = platformConfig.oauthClient.token_expires_at;
      ...
      if (ext.webhookRegistry.isInitialized && ext.webhookRegistry.isSubscribeOnInstall) {
        const client = await ext.getPlatformClient(company_id, session); // eslint-disable-line camelcase
        await ext.webhookRegistry.syncEvents(client, null, true).catch((err) => {
          logger.error(err);
        });
      }
  ...
  FdkRoutes.post('/fp/uninstall', async (req, res, next) => {
    try {
      const { company_id } = req.body; // eslint-disable-line camelcase
      let sid;
      if (!ext.isOnlineAccessMode()) {
  ...
  FdkRoutes.get('/adm/install', async (req, res, next) => {
    try {
      ...
      const partnerConfig = ext.getPartnerConfig(organizationId);
      const session = new Session(Session.generateSessionId(true));
      const sessionExpires = new Date(Date.now() + 900000);
      ...
      const authCallback = urljoin(ext.base_url, '/adm/auth');

      const redirectUrl = partnerConfig.oauthClient.startAuthorization({
        scope: session.scope,
        redirectUri: authCallback,
        state: session.state,
        access_mode: 'online',
  ...
  FdkRoutes.get('/adm/auth', partnerSessionMiddleware(false), async (req, res, next) => {
    try {
      ...
      const token = partnerConfig.oauthClient.raw_token;
      const sessionExpires = new Date(Date.now() + token.expires_in * 1000);
      ...
      if (!ext.isOnlineAccessMode()) {
        ...
        const offlineTokenRes = await partnerConfig.oauthClient.getOfflineAccessToken(ext.scopes, req.query.code);
      ...
      const redirectUrl = urljoin(ext.base_url, '/admin');
      logger.debug(`Redirecting after auth callback to url: ${redirectUrl}`);
      res.redirect(redirectUrl);
    } catch (error) {
      logger.error(error);
