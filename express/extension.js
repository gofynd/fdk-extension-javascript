...
class Extension {
  ...
  async initialize(data) {
    ...
    if (!data.callbacks || (data.callbacks && (!data.callbacks.auth || !data.callbacks.uninstall))) {
      throw new FdkInvalidExtensionConfig(
        'Missing some of callbacks. Please add all `auth` and `uninstall` callbacks.',
      );
    }
    ...
    } else data.cluster = this.cluster;
  ...
  verifyScopes(scopes, extensionData) {
    const missingScopes = scopes.filter(
      (val) => extensionData.scope.indexOf(val) === -1,
    );
    if (!scopes || scopes.length <= 0 || missingScopes.length) {
      throw new FdkInvalidExtensionConfig(
        `Invalid scopes in extension config. Invalid scopes: ${missingScopes.join(', ')}`,
      );
    }
    return scopes;
  }
  ...
  async getPlatformClient(companyId, session) {
    ...
    if (!session.access_token_validity || session.refresh_token) {
      const accessTokenExpired = !session.access_token_validity
        ? true
        : ((session.access_token_validity - new Date().getTime()) / 1000) <= 120;
      if (accessTokenExpired) {
        logger.debug(`Renewing access token for company ${companyId}`);
        const renewTokenRes = await platformClient.config.oauthClient.renewAccessToken(
  ...
  async getPartnerClient(organizationId, session) {
    ...
    if (!session.access_token_validity || session.refresh_token) {
      const accessTokenExpired = ((session.access_token_validity - new Date().getTime()) / 1000) <= 120;
      if (accessTokenExpired) {
        logger.debug(`Renewing access token for organization ${organizationId}`);
        const renewTokenRes = await partnerClient.config.oauthClient.renewAccessToken(
          session.access_mode === 'offline',
        );
  ...
  async getExtensionDetails() {
    ...
    try {
      ...
      const extensionData = await fdkAxios.request(rawRequest);
